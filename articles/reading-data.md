...
requires a data frame with a fixed set of columns and a small number of
...
The contract that downstream functions such as
[`build_bipartite()`](https://saqr.me/bibnets-pkg/reference/build_bipartite.md)
and
[`author_network()`](https://saqr.me/bibnets-pkg/reference/author_network.md)
rely on is that each multi-valued field is stored as a list-column whose
elements are character vectors.

This vignette covers all nine readers, the
[`read_biblio()`](https://saqr.me/bibnets-pkg/reference/read_biblio.md)
entry point, the generic-CSV path, the
[`split_field()`](https://saqr.me/bibnets-pkg/reference/split_field.md)
helper, and the manual construction of bibnets-compatible data frames.
...
pass `format` explicitly or use `format = "generic"` with `actors`.
...
str(oa, max.level = 1)
#> 'data.frame':    1508 obs. of  13 variables:
...
#>  $ authors       :List of 1508
#>  $ references    :List of 1508
#>  $ keywords      :List of 1508
#>  $ affiliations  :List of 1508
#>  $ countries     :List of 1508
...
abstracts from OpenAlex are obtained via
[`openalexR::oa_fetch()`](https://docs.ropensci.org/openalexR/reference/oa_fetch.html)
and processed with
[`read_openalex()`](https://saqr.me/bibnets-pkg/reference/read_openalex.md)
...
reader, demonstrated in section 3, applies to the file format produced
by the OpenAlex web interface. References and abstracts are not present
in this format.
...
- All network builders
  ...
  [`institution_network()`](https://saqr.me/bibnets-pkg/reference/institution_network.md),
  [`conetwork()`](https://saqr.me/bibnets-pkg/reference/conetwork.md))
  accept the same set of arguments (`type`, `counting`, `similarity`,
  `threshold`, `top_n`, `format`), so switching between network types on
  data already in the standard schema requires only a function-name
  change.
